...
* TEST FEATURE SetUp
*******************************************************/
class BehaviorExploreTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    factory_robot = new csci3081::FactoryRobot();
    factory_light = new csci3081::FactoryLight();
    explore = new csci3081::BehaviorExplore();
    ...
    wv_before = *wv_ptr;
    explore->getWheelVelocity(bv->get_sensor_reading_right(light),
  ...
  csci3081::BehaviorExplore * explore;
 ...
 * Test Cases
 ******************************************************************************/
TEST_F(BehaviorExploreTest, Initial) {
  EXPECT_EQ(wv_before.left, 0.0);
  EXPECT_EQ(wv_before.right, 0.0);
}

TEST_F(BehaviorExploreTest, After) {
